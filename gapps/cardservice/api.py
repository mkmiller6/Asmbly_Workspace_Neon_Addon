...
from .constants import (BorderType, ComposedEmailType, ContentType,
                        ...
                        TextButtonStyle, UpdateDraftBodyType, DateTimePickerType)
...
class IconImage:
    alt_text: str = None
    icon: Icon = field(
        metadata=config(encoder=lambda x: x.value, decoder=Icon),
        default=Icon.NONE)
    icon_url: str = ''
...
class DatePicker:
    ...
    type: DateTimePickerType = field(
        metadata=config(encoder=lambda x: x.value, decoder=DateTimePickerType),
        default=DateTimePickerType.DATE_ONLY)
    value_in_ms_since_epoch: int = field(metadata=config(field_name="valueMsEpoch"), default=0)
    ...
    def build(self):
        ...
        for section in self.section:
            ...
            for widget in section.widget:
                ...
                if isinstance(widget, (DatePicker, DateTimePicker, TimePicker)):
                    name = "dateTimePicker"
